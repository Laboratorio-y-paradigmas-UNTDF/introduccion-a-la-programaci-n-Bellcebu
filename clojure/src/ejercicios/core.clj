...
(defn rango-lista
  ...
  [coll]
  (let [extremos (reduce (fn [acc x]
                           {:min (if (< x (:min acc)) x (:min acc))
                            :max (if (> x (:max acc)) x (:max acc))})
                         {:min (first coll) :max (first coll)}
                         (rest coll))]
    (assoc extremos :rango (- (:max extremos) (:min extremos)))))
